...
def DisplayRecentScores(RecentScores):
  ...
  for Count in range(1, NO_OF_RECENT_SCORES + 1):
    if RecentScores[Count].Date != None:
      ...
      Date = "N/A"
    print("{0:<10} {1:<10} {2:10}".format(RecentScores[Count].Name,RecentScores[Count].Score, Date))
  print()
...
def UpdateRecentScores(RecentScores, Score):
  ...
  No = ['n','N','No','no']
  AddingScore = False
  while not AddingScore:
    HighScore = input("Do you want to add your score to the high score table? (y or n): ")
    if HighScore in No:
      HighScore == 'n'
      print()
      print("Your score will not be added to the score table.")
      print()
      AddingScore = True
    elif HighScore in Yes:
      HighScore == 'y'
